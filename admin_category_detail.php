...
<head>
    ...
    <title>Category Detail | Hamro Pasal</title>
</head>
...
<body class="d-flex flex-column h-100">
    ...
    <?php
        $f_id = $_GET["f_id"];
        $query = "SELECT s.s_id,s.s_name,f.f_name,f.f_price,f.f_pic,f.f_desc,f.f_qty
        FROM category f INNER JOIN shop s ON f.s_id = s.s_id WHERE f.f_id = $f_id LIMIT 0,1;";
    ...
    <div class="container px-5 py-4" id="shop-body">
        <?php
            ...
            <div class="row row-cols-1 notibar">
                <div class="col mt-2 ms-2 p-2 bg-success text-white rounded text-start">
                    ...
                    <span class="ms-2 mt-2">Successfully updated Product detail.</span>
                </div>
            </div>
            ...
            <div class="row row-cols-1 notibar">
                <div class="col mt-2 ms-2 p-2 bg-danger text-white rounded text-start">
                    <i class="bi bi-x-circle ms-2"></i><span class="ms-2 mt-2">Failed to update Product detail.</span>
                </div>
            </div>
        ...
        <div class="container row row-cols-6 row-cols-md-12 g-5 pt-4 mb-4" id="shop-header">
            <div class="rounded-25 col-6" id="shop-img" style="
                    background: url(
                        <?php
                            if(is_null($category_row["f_pic"])){echo "'../img/default.png'";}
                            else{echo "'../img/{$category_row['f_pic']}'";}
                        ?> 
                    ) center; height: 225px;
            ...
            <div class="col-6">
                <h1 class="display-5 strong"><?php echo $category_row["f_name"];?></h1>
                <h3 class="display-10 fw-light"><?php echo $category_row["f_desc"];?></h3>
                <h3 class="fw-light" style="color:red"><?php echo $category_row["f_price"]?> NRP</h3>
                <h5 class="display-15 fw-light">Quantity x <?php echo $category_row["f_qty"];?></h5>
                <ul class="list-unstyled">
                    <li class=""><?php echo "from ".$category_row["s_name"];?></li>
                    
                </ul>
        <a class="btn btn-sm btn-primary mt-2 mt-md-0" href="admin_category_edit.php?s_id=<?php echo $category_row["s_id"]?>&f_id=<?php echo $f_id?>">
            <i class="bi bi-pencil-square"></i>
            Update this Product
        </a>
        <a class="btn btn-sm btn-danger mt-2 mt-md-0" href="admin_category_delete.php?f_id=<?php echo $f_id?>">
        ...
        <h3 class="border-top pt-3 mt-2">Orders</h3>
            <form class="form-floating mb-3" method="GET" action="admin_category_detail.php">
                ...
                <div class="row g-2">
                    ...
                    <div class="col">
                        <select class="form-select" id="utype" name="ut">
                            ...
                            <option value="STD" <?php if($_GET["ut"]=="CUS"){ echo "selected";}?>>Regular_Customer</option>
                    ...
                    <div class="col-auto">
                        ...
                        <button type="reset" class="btn btn-danger"
                            onclick="javascript: window.location='admin_category_detail.php?f_id=<?php echo $f_id?>'">Clear</button>
                    </div>
                </div>
            </form>
        </div>
        ...
        <div class="container align-items-stretch">
            ...
            <table class="table rounded-5 table-light table-striped table-hover align-middle caption-top mb-3">
                <caption><?php echo $numrow;?> order(s) <?php if(isset($_GET["search"])){?><br /><a
                        href="admin_category_detail.php?f_id=<?php echo $f_id?>" class="text-decoration-none text-danger">Clear Search
        ...
        <div class="row">
            <div class="col m-2 p-2 bg-danger text-white rounded text-start">
                <i class="bi bi-x-circle ms-2"></i><span class="ms-2 mt-2">No order found with this Product</span>
                <?php if(isset($_GET["search"])){ ?>
                <a href="admin_category_detail.php?f_id=<?php echo $f_id;?>" class="text-white">Clear Search Result</a>
                <?php } ?>
            </div>
        </div>
